...
/*
	With a BTree, we can search through large amounts of data
	by loading only 4 pages from disk

	Methods for reeading and writing to an internal node:
*/
...
/*
	To keep tree balanced, we evenly distribute cells between two new nodes.
	If a leaf node can hold N cells, the during a split we need to 
	distribute N+1 cells between two nodes (N original cells plus one new one)
*/
const uint32_t LEAF_NODE_RIGHT_SPLIT_COUNT = (LEAF_NODE_MAX_CELLS + 1) / 2;
const uint32_t LEAF_NODE_LEFT_SPLIT_COUNT = (LEAF_NODE_MAX_CELLS + 1) - LEAF_NODE_RIGHT_SPLIT_COUNT;

// new pages will alwats go onto the end of the database file
...
void create_new_root(Table* table, uint32_t right_child_page_num) {
	/*
		...
		Old root copied to new page and becomes left child.
		Address of right child is passed in.
		Re-initialize root page to contain the new root node.
		New rot node points to two children
	*/
...
void internal_node_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num) {
	...
	*internal_node_num_keys(parent) = original_num_keys + 1;
	...
	if (original_num_keys >= INTERNAL_NODE_MAX_CELLS) {
		printf("Nede to implement splitting internal node\n");
		exit(EXIT_FAILURE);
	}
...
void leaf_node_split_and_insert(Cursor* cursor, uint32_t key, Row* value) {
	...
	*(leaf_node_num_cells(new_node)) = LEAF_NODE_RIGHT_SPLIT_COUNT;
	...
	//Then we need to update node's parent. If it is root node, then create new root
...
//we want to search the table for the correct place to insert, then insert there
...
/*
	Recursive function that takes any node and prints it and its childrem.
	It takes an indentation level as a parameter, which increases with each recursive call.
	There is also a small helper function to indent
*/
